...
// We must replace `<div key={block.id} data-block` with `<React.Fragment key={block.id}><div data-block`
content = content.replace(
  /<div key=\{block\.id\} data-block/g,
  '<React.Fragment key={block.id}><div data-block'
);

const regexEndOfBlock = /(Контент ниже скрыт до взаимодействия[\s\S]*?<\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\);\s*\}\)\s*<\/div>)/;

content = content.replace(regexEndOfBlock, function(match) {
  // We want to inject the div before the `);\n            })}` 
