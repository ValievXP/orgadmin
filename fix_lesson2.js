...
// Inject indicator only around \`<div key={block.id} data-block>\` which is specific to LessonEditorPage.
content = content.replace(
  /<div key=\{block\.id\} data-block/g,
  '<React.Fragment key={block.id}><div data-block'
);

// We find the exact end of that div.
const endOfBlock = `                  {(block.type==='button'||block.type==='exercise')&&block.data.isDivider && (
...
const newEndOfBlock = `                  {(block.type==='button'||block.type==='exercise')&&block.data.isDivider && (
                    <div className="px-4 pb-3">
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200 text-amber-700 text-[11px] font-medium">
                        <EyeOff className="w-3.5 h-3.5" /> Контент ниже скрыт до взаимодействия
                      </div>
...
content = content.replace(endOfBlock, newEndOfBlock);
