...
const regexEndOfBlock = /\s*\)\}\s*<\/div>\s*\);\s*\}\)\s*<\/div>/g;
let replaced = false;

content = content.replace(regexEndOfBlock, function(match, offset, string) {
  // Only replace the SECOND occurrence (which is the one in LessonEditorPage, not ColumnsBlockEditor)
  // Wait, the one in ColumnsBlockEditor is `</div>\n                  );\n                })}` which doesn't match exactly because of `)} </div>` vs `</div>`
...
// Fix if it replaced both (we only want one closing React.Fragment inside ColumnsBlockEditor but there is none)
// Wait, ColumnsBlockEditor's end is: `                  </div>\n                  );\n                })}`
// It DOES NOT HAVE `)}` before the div!
// My regex: `\s*\)\}\s*<\/div>\s*\);\s*\}\)\s*<\/div>`
// This perfectly matches ONLY the end of LessonEditorPage `blocks.map`!
