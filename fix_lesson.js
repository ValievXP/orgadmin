...
// 3. Add Palette (Text Color) to toolbars
// In TextBlockEditor
...
// 4. Replace TableBlockEditor
const tableEditorRegex = /function TableBlockEditor\(\{ data, onChange \}: \{ data: any; onChange: \(d: any\) => void \}\) \{[\s\S]*?(?=function ColumnsBlockEditor)/g;

const newTableEditor = `function TableBlockEditor({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  const cells: {html:string, color?:string}[][] = data.cells || [[{html:''},{html:''}],[{html:''},{html:''}]];
  const colWidths: string[] = data.colWidths || Array(cells[0]?.length || 2).fill('auto');
  ...
  const updateColWidth = (c:number, width:string) => {
