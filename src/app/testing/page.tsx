...
const INITIAL_COURSES = [
  { 
    id: 'TST-821', title: 'Оценочное тестирование навыков AI', lang: 'RUS', status: 'Active', inCatalog: true, format: 'Quiz',
    users: 142, modules: 12, lessons: 48, parentId: null,
    cover: 'bg-gradient-to-br from-indigo-400 to-purple-600'
  },
  { 
    id: 'TST-724', title: 'Введение в управление проектами (Тест)', lang: 'UZB', status: 'Draft', inCatalog: false, format: 'Standard',
    users: 0, modules: 8, lessons: 24, parentId: null,
    cover: 'bg-gradient-to-br from-slate-200 to-slate-400'
  },
  { 
    id: 'TST-612', title: 'Определение архетипа личности', lang: 'RUS', status: 'Active', inCatalog: true, format: 'Quiz',
    users: 89, modules: 24, lessons: 120, parentId: null,
    cover: 'bg-gradient-to-br from-fuchsia-500 to-pink-600'
  },
  { 
    id: 'TST-509', title: 'Английский язык (Средний уровень)', lang: 'RUS', status: 'Archived', inCatalog: false, format: 'Standard',
    users: 512, modules: 40, lessons: 240, parentId: null,
    cover: 'bg-gradient-to-br from-orange-400 to-rose-500'
  },
  { 
    id: 'TST-990', title: 'Анкета: Знакомство с компанией', lang: 'RUS', status: 'Active', inCatalog: true, format: 'Quiz',
    ...
    return (
      ...
      >
        ...
        <div className="col-span-3 flex flex-wrap gap-2 items-center">
          ...
          {course.format === 'Quiz' && (
             <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider backdrop-blur-md shadow-sm bg-indigo-50 text-indigo-600">
               Квиз
             </span>
          )}
        </div>
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1 min-w-0">
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          {course.format === 'Quiz' && (
             <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider shadow-sm bg-indigo-50 text-indigo-600 border border-indigo-200">
               Квиз
             </span>
          )}
        </div>
...
export default function TestingPage() {
  ...
  return (
    <div className="flex flex-col min-h-full w-full bg-[#F9FAFB] dark:bg-[var(--bg-app)] overflow-x-hidden relative">
      <PageHeader 
        title={!breadcrumbs ? "Тестирования" : undefined}
            ...
            <>
              ...
              {(visibleCourses.length > 0 || !isSearching) && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-75">
                  <h2 className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-4">{isSearching ? 'Результаты поиска' : 'Тестирования'}</h2>
                    ...
                    {viewMode === 'grid' ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {!isSearching && (
                          ...
                          >
                            ...
                            <h3 className="font-bold text-neutral-900 transition-colors mb-1 text-lg">Создать тестирование</h3>
                      ...
                      <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm flex flex-col">
                        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-neutral-100 bg-neutral-50/50 rounded-t-2xl text-[11px] font-bold text-neutral-400 uppercase tracking-wider items-center">
                          <div className="col-span-4">Тестирование</div>
                        ...
                        <div className="flex flex-col divide-y divide-neutral-100 relative min-h-[100px]">
                           {visibleCourses.length === 0 ? (
                              <div className="py-12 flex flex-col items-center justify-center text-neutral-500"><p>Здесь пока нет тестирований</p></div>
      ...
      {deleteCourseId && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
           ...
           <div className="bg-white rounded-[24px] w-full max-w-[420px] shadow-2xl relative z-10 p-8 animate-in zoom-in-95 duration-200 text-center flex flex-col items-center">
              ...
              <h2 className="text-[22px] font-bold text-neutral-900 mb-3 tracking-tight">Удалить тестирование?</h2>
              <p className="text-[15px] text-neutral-500 mb-8 leading-relaxed max-w-[340px]">Вы действительно хотите безвозвратно удалить тестирование <strong className="text-neutral-900 font-semibold">"{courses.find(c => c.id === deleteCourseId)?.title}"</strong>?</p>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl w-full max-w-[720px] shadow-2xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
             <div className="p-8 pb-6 border-b border-neutral-100 flex items-center justify-between shrink-0 bg-white z-20">
               <div>
                 <h2 className="text-2xl font-black text-neutral-900 tracking-tight">{courseEditor.mode === 'create' ? 'Создание тестирования' : 'Настройки тестирования'}</h2>
                 <p className="text-sm text-neutral-500 mt-1">{courseEditor.mode === 'create' ? 'Заполните основные данные о новом материале' : 'Отредактируйте основные данные о тестировании'}</p>
               </div>
             </div>
