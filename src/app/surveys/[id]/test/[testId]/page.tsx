...
export default function TestEditorPage() {
  ...
  const initQuestions: Question[] = isNew ? [{ id: 'q1', type: 'radio', text: '', image: null, answers: [{ id: 'o1', text: '', isCorrect: true, image: null, explanation: '' }, { id: 'o2', text: '', isCorrect: false, image: null, explanation: '' }] }] : MOCK_QUESTIONS;
              ...
              <div className="space-y-1 divide-y divide-neutral-100">
                ...
                {settings.timeLimit && (
                  <div className="pt-3 pb-2 pl-11 animate-in fade-in slide-in-from-top-1 duration-200">
                    <label className="block text-[11px] font-semibold text-neutral-400 uppercase tracking-wider mb-1.5">Лимит на прохождение</label>
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <input 
                           type="number" min={0} value={settings.timeLimitHours} 
                           onChange={e => updateSettings({ timeLimitHours: Number(e.target.value) })} 
                           className="w-24 h-10 px-3 pr-8 rounded-lg border border-neutral-200 text-sm text-neutral-900 bg-neutral-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[var(--color-admin-primary-500)] focus:border-transparent transition-all font-semibold" 
                         />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-neutral-500 font-medium pointer-events-none">ч</span>
                      </div>
                      <span className="text-neutral-300 font-medium">:</span>
                      <div className="relative">
                        <input 
                           type="number" min={0} max={59} value={settings.timeLimitMinutes} 
                           onChange={e => updateSettings({ timeLimitMinutes: Number(e.target.value) })} 
                           className="w-24 h-10 px-3 pr-10 rounded-lg border border-neutral-200 text-sm text-neutral-900 bg-neutral-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[var(--color-admin-primary-500)] focus:border-transparent transition-all font-semibold" 
                         />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-neutral-500 font-medium pointer-events-none">мин</span>
                      </div>
                    </div>
                  </div>
                )}
