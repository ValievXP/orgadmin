...
// [I am including the exact code the user provided]
            ...
            {React.Children.map(children, (child) => {
              if (React.isValidElement(child) && (child.props as any).value === activeId) {
                return React.cloneElement(child as React.ReactElement<any>, {
                  ...(child.props as any),
                  className: cn((child.props as any).className, 'z-50 shadow-lg scale-105 opacity-80 cursor-grabbing'),
                });
              }
              return null;
            })}
  ...
  return (
    <SortableItemContext.Provider value={{ listeners, isDragging: isSortableDragging, disabled }}>
      <Comp
        ...
        ref={setNodeRef}
        style={style}
        className={cn(isSortableDragging && 'opacity-0', disabled && 'opacity-50', className)}
      >
        {children}
      </Comp>
